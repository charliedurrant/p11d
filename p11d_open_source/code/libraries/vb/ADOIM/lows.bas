...
Public Function addcriteriaequal(ByVal ispec As ImportSpec) As String
   ...
   ElseIf (ispec.vartype = TYPE_DOUBLE) Or (ispec.vartype = TYPE_LONG) Then
     addcriteriaequal = addcriteriaequal & CStr(ispec.Value)
   ElseIf ispec.vartype = TYPE_DATE Then
     addcriteriaequal = addcriteriaequal & UniversalDateFormat(ispec.Value)
   ElseIf ispec.vartype = TYPE_STR Then
     addcriteriaequal = addcriteriaequal & "'" & ispec.Value & "'"
   Else
...
' #yyyy-mm-dd hh:mm:ss#.
Public Function UniversalDateFormat(ByVal vDate As Variant) As String
  If IsNull(vDate) Then
    UniversalDateFormat = "Null"
  Else
   UniversalDateFormat = "#" & Format$(vDate, "yyyy-mm-dd") & "#"
  End If
End Function
...
Public Function DebugRecord(ByVal rs As Recordset) As String
  Dim i As Long, iLine As Long
  
  On Error Resume Next
  If rs Is Nothing Then
    DebugRecord = "(Recordset is nothing)"
  Else
    If rs.EOF And rs.BOF Then
      DebugRecord = DebugRecord & "(EOF_BOF)" & vbCrLf
    Else
      iLine = 0
      DebugRecord = ""
      For i = 0 To (rs.Fields.Count - 1)
        DebugRecord = DebugRecord & "[" & rs.Fields(i).Name & "] = "
        DebugRecord = DebugRecord & IsNullEx(rs.Fields(i).Value, "(Null)")
        iLine = iLine + 1
        If (iLine Mod 3) = 0 Then
          DebugRecord = DebugRecord & vbCrLf
        Else

...
Begin VB.Form frmPrintDialog 
   ...
   Begin VB.Frame fraPrinterName 
      ...
      Top             =   0
      Width           =   6135
      Begin VB.ComboBox cmbPrinterPrv 
         Height          =   315
         Left            =   120
         TabIndex        =   18
         Top             =   240
...
err_End:
  Exit Sub
err_Err:
  Call ErrorMessage(ERR_ERROR, Err, "Load", "Load", "Failed to load the print dialog")
  Resume err_End
End Sub
...
Private Sub cmbPrinterPrv_Click()
  
  On Error GoTo err_Err
  
  Call SetActivePrinter(cmbPrinterPrv.Text)
...
Private Sub SetActivePrinterReporter()
  ...
  Call rep.SetActivePrinter(cmbPrinterPrv.Text)
End Sub
...
Private Sub SetPrinters()
  ...
  For Each prt In Printers
    cmbPrinterPrv.AddItem (prt.DeviceName)
  Next
  
  If cmbPrinterPrv.ListCount > 0 Then
    cmbPrinterPrv.Text = Printer.DeviceName
  Else
    cmbPrinterPrv.AddItem ("No printers available")
    cmbPrinterPrv.Text = "No printers available"
  End If

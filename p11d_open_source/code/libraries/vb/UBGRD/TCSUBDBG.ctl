...
Begin VB.UserControl UBGRD 
   ...
   ScaleHeight     =   3600
   ScaleWidth      =   4800
   ToolboxBitmap   =   "TCSUBDBG.ctx":0000
   Begin TrueDBGrid60.TDBGrid Grid_i 
      Height          =   2850
      Left            =   180
      OleObjectBlob   =   "TCSUBDBG.ctx":00FA
      TabIndex        =   0
      Top             =   225
      Width           =   4335
   End
...
Public Grid As Object 'VB6 Change TDBGrid

Public Event ReadData(RowBuf As TrueDBGrid60.RowBuffer, ByVal RowBufRowIndex As Long, ObjectList As ObjectList, ByVal ObjectListIndex As Long)
Public Event WriteData(ByVal RowBuf As TrueDBGrid60.RowBuffer, ByVal RowBufRowIndex As Long, ObjectList As ObjectList, ObjectListIndex As Long)
Public Event ValidateTCS(FirstColIndexInError As Long, ValidateMessage As String, ByVal RowBuf As TrueDBGrid60.RowBuffer, ByVal RowBufRowIndex As Long, ByVal ObjectListIndex As Long)
Public Event DeleteData(ObjectList As ObjectList, ObjectListIndex As Long)

' TDBGrid events raised
...
Private Sub Grid_i_UnboundDeleteRow(Bookmark As Variant)
  If Not m_OL Is Nothing Then RaiseEvent DeleteData(m_OL, CLng(Bookmark))
End Sub
...
Public Property Let DisplayErrors(ByVal NewValue As Boolean)
...
Public Property Set ObjectList(NewValue As ObjectList)
  Set m_OL = Nothing
  If NewValue Is Nothing Then Grid_i.ClearFields
  Grid_i.DataChanged = False
  Grid_i.Bookmark = Null
  Set m_OL = NewValue
  Grid_i.ReBind
  Grid_i.Bookmark = Null
End Property
...
Private Sub grid_i_UnboundReadDataEx(ByVal RowBuf As TrueDBGrid60.RowBuffer, StartLocation As Variant, ByVal offset As Long, ApproximatePosition As Long)
  ...
  Do While (ObjectListIndex > 0) And (RowBufRowIndex < RowBuf.RowCount)
    ...
    If RowBuf.ColumnCount = 0 Then Exit Do
...
Private Sub grid_i_UnboundAddData(ByVal RowBuf As TrueDBGrid60.RowBuffer, NewRowBookmark As Variant)
  Dim NewObjectListIndex As Long
  Dim ErrMsg As String
  Dim FirstColIndexInError As Long
  
  FirstColIndexInError = -1
  RaiseEvent ValidateTCS(FirstColIndexInError, ErrMsg, RowBuf, 0, -1)
  If FirstColIndexInError = -1 Then
    NewObjectListIndex = -1
    
    RaiseEvent WriteData(RowBuf, 0, m_OL, NewObjectListIndex)
  ...
  Else
    ...
    If m_bDisplayErrors Then
      Call ErrorMessage(ERR_INFO, Err, "UnboundAddData", "Error in column " & FirstColIndexInError & ". ", ErrMsg)
    End If
  End If
End Sub

Private Sub grid_i_UnboundWriteData(ByVal RowBuf As TrueDBGrid60.RowBuffer, WriteLocation As Variant)
  Dim FirstColIndexInError As Long
  Dim ObjectListIndex As Long, i As Long
  Dim ErrMsg As String
  
  FirstColIndexInError = -1
  If IsNumeric(WriteLocation) Then
    ObjectListIndex = CLng(WriteLocation)
    RaiseEvent ValidateTCS(FirstColIndexInError, ErrMsg, RowBuf, 0, ObjectListIndex)
    If FirstColIndexInError = -1 Then
      RaiseEvent WriteData(RowBuf, 0, m_OL, ObjectListIndex)
    Else
      
      If m_bDisplayErrors Then
        Call ErrorMessage(ERR_ERROR, Err, "UnboundAddData", "Error in column " & FirstColIndexInError & ". ", ErrMsg)
      End If
    End If
  Else
    Call Err.Raise(ERR_UNBOUNDWRITE, "UnboundWriteData", "The object list index is not numeric.")
  End If
End Sub

Private Function GetObjectListIndex(oList As ObjectList, RowBuf As TrueDBGrid60.RowBuffer, StartLocation As Variant, ByVal offset As Long) As Long
  ...
  If IsNull(StartLocation) Then
    ...
    Else
      GetObjectListIndex = -1
      GoTo GetObjectListIndex_End
    End If
    If FirstObject < 0 Then Exit Function
  Else
    FirstObject = StartLocation
    If m_OL(FirstObject) Is Nothing Then
      GetObjectListIndex = -1
      GoTo GetObjectListIndex_End
    End If
  End If
...
Private Sub Grid_i_RowColChange(LastRow As Variant, ByVal LastCol As Integer)

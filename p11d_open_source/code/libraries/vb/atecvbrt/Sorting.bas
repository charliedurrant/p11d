...
Public Sub SortAny(v As Variant, ByVal lmin As Long, ByVal lmax As Long, SortFn As ISortFunction, ByVal st As TCS_SORTTYPE)
  ...
  If lmax > lmin Then
    pSA = GetPtrToSA(v)
    If pSA = 0 Then Err.Raise ERR_SORT, "SortAny", "Cannot sort a variant that is not an array"
    SizeOfElement = SafeArrayGetElemsize(pSA)
    If SizeOfElement > MAX_ELEMENT_SIZE Then Err.Raise ERR_SORT, "SortAny", "Array element size must be less than " & CStr(MAX_ELEMENT_SIZE)
    ...
    If pSafeArrayData <> 0 Then
      ...
      Else
        Err.Raise ERR_SORT, "SortAny", "Unknown Sort type [" & CStr(st) & "]"
      End If
    End If
  End If
End Sub

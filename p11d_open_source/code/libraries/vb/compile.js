...
try
{
  ...
  if ( scriptpath != "c:\\vb6\\library" )
  { throw "The script is not running from c:\\vb6\\library." ; }  
...
function processprojects(projects)
{
  ...
  try
  {
    ...
    for ( i = 0 ; i < projects.length; i++)
    {
      ...
      echoex("unregistering: " + project.component);
      m_shell.Run(winsyspath + "regsvr32.exe /u /s " + project.component,8,true);

      echoex("copying to release: " + project.component);
      fs.CopyFile ( project.pathproject + project.component , project.pathrelease + project.component  , true ) ;

      echoex("copying to system: " + project.component);
      if ( fs.FileExists(winsyspath + project.component))
      {
        fattr = fs.GetFile(winsyspath + project.component);
        if (fattr.attributes & 1)
        { fattr.attributes = fattr.attributes - 1; }
      }
      fs.CopyFile ( project.pathproject + project.component , winsyspath + project.component, true ) ;
      echoex("registering: " + winsyspath + project.component);
      ret = m_shell.Run(winsyspath +"regsvr32.exe /s " + winsyspath + project.component,8,true);
  ...
  try
  {
    while (! ts.AtEndOfStream )
    {
      ...
      if ( ( sline.length > 0 ) && ( sline.substring(0, 1) != ";" ) )
      {
        ...
        { throw "The project folder " + o.pathproject + " does not exist."; }

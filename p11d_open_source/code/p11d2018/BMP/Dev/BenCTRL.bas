        ...
        If (IsCBDEmployee(benParent.value(ee_PersonnelNumber_db))) Then
          ...
          For i = 1 To employees.Count
            ...
            If ee.BenefitsLoaded Then
              If ee.benefits.ItemIndex(ben) > 0 Then
                Set benEE = ee
                If Not benEE.NeedToCalculate Then
...
Public Sub BenefitAddNewRecord(ben As IBenefitClass, rs As Recordset)
  ...
  If (p11d32.BringForward.Yes And Not ben.ReadFromDB) Or (Len(ben.RSBookMark) = 0) Then
    rs.AddNew
    rs.Fields(S_EMPLOYEE_NUM_FIELD) = GetEmployeeNumber(ben)
  Else
...
TRY_AGAIN:
  l = l + 1
  If l > 1 Then Set benX = benX.Parent
...
Public Function CorrectBenValue(bc As BEN_CLASS, Item As Long, value As Variant) As Variant
  ...
  CorrectBenValue = GetTypedValue(value, p11d32.BenDataLinkDataType(bc, Item))
...
Public Sub SetStandardBenItemsDataTypes(ByVal bc As BEN_CLASS)
  ...
  p11d32.BenDataLinkDataType(bc, ITEM_NIC_CLASS1A_VALUE) = TYPE_DOUBLE
...
Public Sub StandardWriteData(ben As IBenefitClass, rs As Recordset)
...
Public Sub StandardReadData(ben As IBenefitClass, Optional rs As Recordset = Nothing)
  Dim Rates As Rates
  Dim bc As BEN_CLASS
  ...
  Else
    ...
    If bc > 0 Then
      ...
      Else
        If Not rs Is Nothing Then ben.value(ITEM_MADEGOOD_IS_TAXDEDUCTED) = rs.Fields("MadeGoodIsTaxDeducted")
      End If
    Else
  ...
  End If
End Sub
Public Sub BenCalcNIC(ben As IBenefitClass, Optional INCV As BASE_ITEMS = ITEM_NIC_CLASS1A_VALUE, Optional INCB As BASE_ITEMS = ITEM_NIC_CLASS1A_BENEFIT, Optional IV As BASE_ITEMS = ITEM_VALUE, Optional IB As BASE_ITEMS = ITEM_BENEFIT)
  Dim d As Double
  ...
  'FC - Class1A
  'If Not CheckIfNotSubjectToClass1A(ben) Then RK 25/06/03 Logical flaw?
  If CheckIfNotSubjectToClass1A(ben) Then
    ben.value(ITEM_CLASS1A_ADJUSTMENT) = 0
    ben.value(ITEM_NIC_CLASS1A_BENEFIT) = 0
    ben.value(ITEM_NIC_CLASS1A_VALUE) = 0
    Exit Sub
  End If

  'FIX XX
  d = p11d32.Rates.value(carNICRate)
  ben.value(INCV) = ben.value(IV) * d
  ben.value(INCB) = ben.value(IB) * d
  If ben.value(ITEM_NIC_CLASS1A_ABLE) Then
    If ben.value(ITEM_MADEGOOD_IS_TAXDEDUCTED) Then
     'FIX XX
      ben.value(ITEM_CLASS1A_ADJUSTMENT) = ben.value(ITEM_MADEGOOD_NET)
    End If
  End If
BenCalcNIC_END:
  Exit Sub
...
Public Sub SetStandardBenItemsUDMData(ByVal bc As BEN_CLASS, Optional sClass1ANICFieldNameSuffix As String = "")
...
RemoveCDBAssignment_ERR:
  Call ErrorMessage(ERR_ERROR, Err, "RemoveCDBAssignment", "Remove CDB Assignment", "Error removing a company defined benefit.")
...
'FC - Class1A
Public Function CheckIfNotSubjectToClass1A(ByVal ben As IBenefitClass) As Boolean
  Dim bc As BEN_CLASS
  ...
  'If p11d32.AppYear < 2001 Then GoTo CheckIfNotSubjectToClass1A_END
  
  If Not ben.value(ITEM_NIC_CLASS1A_ABLE) Then GoTo CheckIfNotSubjectToClass1A_END
  bc = ben.BenefitClass
  

  Set benEmployee = GetParentFromBenefit(ben, GPBF_EMPLOYEE)

  If benEmployee Is Nothing Then Call Err.Raise(ERR_IS_NOTHING, CheckIfNotSubjectToClass1A, "No Employee exists")

  
  'If Not benEmployee.value(ee_Class1AEmployeeIsNotSubjectTo) Then GoTo CheckIfNotSubjectToClass1A_END 'RK logical flaw?
  If benEmployee.value(ee_Class1AEmployeeIsNotSubjectTo_db) Then GoTo CheckIfNotSubjectToClass1A_END 'RK logical flaw?

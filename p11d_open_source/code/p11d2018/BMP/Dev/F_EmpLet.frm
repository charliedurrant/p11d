...
Private Function FileSave(ByVal sPathAndFile As String, bNewFile As Boolean) As Boolean
  ...
  Set ts = FS.OpenTextFile(sPathAndFile, ForWriting, bNewFile)
  If ts Is Nothing Then Call Err.Raise(ERR_TEXTSTREAM_IS_NOTHING, "FileSave", "The text stream object for " & sPathAndFile & " is nothing.")
  
  ts.Write rtEmpLet.Text
  ts.Close
  m_dirty = False

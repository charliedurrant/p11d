...
Begin VB.Form F_NonSharedVans 
   ...
   Begin VB.Frame fmeInput 
      ...
      Begin VB.CheckBox chkbx 
         Alignment       =   1  'Right Justify
         Caption         =   "Was fuel available for private use?"
         ForeColor       =   &H00800000&
         Height          =   350
         Index           =   7
         Left            =   4320
...
Private Sub Form_Load()
  ...
  Call SetupOpraInput(Label2(2), TxtBx(2), " (Vans)")
  Call SetupOpraInput(Label2(0), TxtBx(8), " (Fuel)")
...
Private Function IBenefitForm2_AddBenefitSetDefaults(ben As IBenefitClass) As Long
  ...
  With ben
    Call StandardReadData(ben, Nothing)
    ...
    .value(nsvan_fuel_available_db) = True
    .value(nsvan_RegistrationDate_db) = p11d32.Rates.value(VanRegDateNew)
    .value(nsvan_is_electric_db) = False
...
Private Function IBenefitForm2_BenefitOn() As Boolean
  With benefit
    ...
    chkbx(7).value = BoolToChkBox(.value(nsvan_fuel_available_db))
    chkbx(8).value = BoolToChkBox(.value(nsvan_is_electric_db))
...
On Error GoTo BenefitsToListView_err
  ...
  TxtBx(2).Text = ben.value(ee_VansOPRAAmountForegone_db)
  TxtBx(8).Text = ben.value(ee_VansFuelOPRAAmountForegone_db)
    ...
    Select Case UCASE$(.Name)
      ...
      Case "TXTBX"
        Select Case .Index
          ...
          Case 2
            bDirty = CheckTextInput(.Text, ben, ee_VansOPRAAmountForegone_db)
            ben.Dirty = ben.Dirty Or bDirty
            ben.InvalidFields = InvalidFields(Me, fmeSharedVans)
          Case 8
            bDirty = CheckTextInput(.Text, ben, ee_VansFuelOPRAAmountForegone_db)
            ben.Dirty = ben.Dirty Or bDirty
            ben.InvalidFields = InvalidFields(Me, fmeSharedVans)
          Case Else
      ...
      Case UCASE$("CHKBX")
        Select Case .Index
          ...
          Case 7
            bDirty = CheckCheckBoxInput(.value, benefit, nsvan_fuel_available_db)
          Case 8
            bDirty = CheckCheckBoxInput(.value, benefit, nsvan_is_electric_db)
          Case Else
            ECASE "Unknown control index"
        End Select
      Case Else

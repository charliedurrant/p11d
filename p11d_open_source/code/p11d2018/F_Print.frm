...
Begin VB.Form F_Print 
   ...
   Begin VB.Frame fraP46Options 
      ...
      Top             =   2655
      Width           =   3390
      Begin VB.CheckBox chkPrintReplacedP46s 
...
Private Sub chkPrintReplacedP46s_Click()
  p11d32.ReportPrint.P46PrintReplacedP46s = ChkBoxToBool(chkPrintReplacedP46s)
End Sub
...
err_end:
  Exit Sub
err_err:
  Call ErrorMessage(ERR_ALLOWIGNORE Or ERR_ERROR, Err, "CheckPrinterSize", "Paper not set to A4", "The paper is not correct")
  Resume err_end
End Sub
Public Sub DoPrint(ByVal rTarget As REPORT_TARGET)
  ...
  Select Case rt
    Case RPTT_STANDARD
      If p11d32.CurrentEmployer.GetEmployees(TempOL) Then
        Set p11d32.ReportPrint.SelectedEmployees = TempOL
          If Not p11d32.CurrentEmployer.DataChecker(rTarget, TempOL) Then
            GoTo DoPrint_END
          End If
          Call SetCursor
...
DoPrint_ERR:
  ...
  Resume
End Sub
Private Function WarningExportReportAsHTMLOrPDF() As Boolean
  If (p11d32.ReportPrint.Destination = REPD_FILE_HTML) Then
    Call MsgBox("You are not able to print 'user' or 'management' reports to HTML or as PDFs. " & vbCrLf & "Please amend your destination selection on the print options form.", vbExclamation, "Functionality not available")
    WarningExportReportAsHTMLOrPDF = True
  End If
End Function
...
Public Sub cmdPrintPreview_Click(Index As Integer)
  ...
  Select Case Index
      Case 0
        If StrComp(cmbPrinter.Text, NO_PRINTERS_AVAILABLE, vbTextCompare) = 0 Then
          Call MsgBox(NO_PRINTERS_AVAILABLE_MSG)
        Else
          Call DoPrint(PRINT_REPORT)
        End If
      Case 1
...
Private Sub SetPrinters()
  ...
  For Each prt In Printers
    F_Print.cmbPrinter.AddItem (prt.DeviceName)
  Next
  
  If cmbPrinter.ListCount > 0 Then
    cmbPrinter.Text = Printer.DeviceName
  Else
    F_Print.cmbPrinter.AddItem (NO_PRINTERS_AVAILABLE)
    cmbPrinter.Text = NO_PRINTERS_AVAILABLE
  End If

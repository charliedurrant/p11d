...
Begin VB.Form F_PayeOnline 
   ...
   Begin VB.Frame frmPAYEOnlineButtons 
      ...
      Top             =   0
      Width           =   9375
      Begin VB.CheckBox chkPerformLocalSchemaValidation 
         Caption         =   "Local schema validation"
         Height          =   255
         Left            =   7080
         TabIndex        =   14
         Top             =   3840
...
Private Sub chkCheckSubmissionAllowed_Click()
 p11d32.PAYEonline.DisableCheckSubmissionWithAbatec = Not ChkBoxToBool(chkCheckSubmissionAllowed)
End Sub

Private Sub chkPerformLocalSchemaValidation_Click()
  p11d32.PAYEonline.ByPassLocalSchemaCheck = Not ChkBoxToBool(chkPerformLocalSchemaValidation)
End Sub
...
Private Sub cmdCancel_Click()
  ...
  If (m_Running) Then
    Msg = "By cancelling the submission you are not retracting your submitted data. It is not possible to recall a submission according to HMRC specifications. We thus advise not cancelling and waiting for a message from the govermenent gateway. If you do wish to cancel you will have to contact HMRC and ask for the submission to be recalled by other means." & vbCrLf & vbCrLf & "Click 'Yes' to cancel or 'No' to continue and wait for a response from the HMRC gateway."
    If MsgBox(Msg, vbYesNo, "Cancel checking") = vbYes Then
      p11d32.PAYEonline.SubmissionCancelled = True
    End If
  Else
    Me.Hide
  End If
...
Private Sub cmdRun_Click()
  ...
  If p11d32.PAYEonline.Type_Selected = POT_P46 Then
    ...
    Call p11d32.Help.ShowForm(F_PayeOnlineP46Options, vbModal)
    If Not F_PayeOnlineP46Options.Cancelled Then
      Call PAYERun
    End If
...
Err_End:
  ...
  If Not F_PayeOnlineP46Options Is Nothing Then
    ...
    Set F_PayeOnlineP46Options = Nothing
  End If
  m_Running = False
  p11d32.PAYEonline.SubmissionCancelled = False
...
Private Sub LicenceDependantSettings()
  'submit P11D
  optOnlineForm(POT_P11D).Enabled = p11d32.ReportPrint.AllReports
  optOnlineForm(POT_P11DB_ONLY).Enabled = p11d32.ReportPrint.AllReports
  If (Not p11d32.ReportPrint.AllReports) Or p11d32.JuneRelease Then
    p11d32.PAYEonline.Type_Selected = POT_P46
  End If
  optOnlineForm(POT_P11D).Visible = Not p11d32.JuneRelease
  optOnlineForm(POT_P11DB_ONLY).Visible = Not p11d32.JuneRelease
...
Private Sub optOnlineForm_Click(Index As Integer)
    ...
    Select Case Index
      ...
      Case POT_P11DB_ONLY
        p11d32.PAYEonline.Type_Selected = POT_P11DB_ONLY

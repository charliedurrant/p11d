...
Public Function StringToJSON(st As String) As String
   ...
   Else
      ...
      For lRecCnt = LBound(rows) To UBound(rows)
         ...
         Next 'fld
         sRecs.Append IIf((Trim(sRecs.bstr) <> ""), "," & vbCrLf, "") & "{" & sFlds & "}"
      Next 'rec
      StringToJSON = ("( {""Records"": [" & vbCrLf & sRecs.bstr & vbCrLf & "], " & """RecordCount"":""" & lRecCnt & """ } )")
   End If
End Function
   ...
   Else
      ...
      Else
         Do While Not rs.EOF And Not rs.BOF
            ...
            rs.MoveNext
         Loop
         RStoJSON = ("( {""Records"": [" & vbCrLf & sRecs.bstr & vbCrLf & "], " & """RecordCount"":""" & lRecCnt & """ } )")
      End If
   End If
